...
namespace NHST.Controllers
{
    public class TransactionController
    {
        ...
        //Đặt cọc thêm đơn hàng hộ
        public static int DepositExtra(int userID, double wallet, DateTime currentDate, string userUserName, int orderID, int statusnew, int orderStatus, string amountdeposit, double custDeposit, string content, int typePayWallet, int tradeType, int typePayOrder)
        {
            using (NHSTEntities dbe = new NHSTEntities())
            {
                dbe.Configuration.ValidateOnSaveEnabled = false;
                using (var transaction = dbe.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        h.UID = userID;
                        h.Username = userUserName;
                        h.HistoryContent = userUserName + " đã đặt cọc thêm cho đơn hàng ID là: " + orderID + "";
                        h.Type = 1;
                        h.CreatedDate = currentDate;
                        dbe.tbl_HistoryOrderChange.Add(h);
