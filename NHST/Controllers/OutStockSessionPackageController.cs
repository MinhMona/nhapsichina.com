...
namespace NHST.Controllers
{
    public class OutStockSessionPackageController
    {
        ...
        public static List<tbl_OutStockUserPackage> GetAllByOutStockUserID(int OutStockSessionID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_OutStockUserPackage> las = new List<tbl_OutStockUserPackage>();
                las = dbe.tbl_OutStockUserPackage.Where(a => a.OutStockUserID == OutStockSessionID).OrderByDescending(a => a.ID).ToList();
                return las;
            }
        }
        ...
        public static string InsertUser(int OutStockUserID, int SmallPackageID, string OrderTransactionCode, int MainOrderID, DateTime CreatedDate, string CreatedBy, double Weight)
        {
            using (var dbe = new NHSTEntities()) //now wrapping the context in a using to ensure it is disposed
            {
                tbl_OutStockUserPackage user = new tbl_OutStockUserPackage();
                user.OutStockUserID = OutStockUserID;
                user.SmallPackageID = SmallPackageID;
                user.OrderTransactionCode = OrderTransactionCode;
                user.MainOrderID = MainOrderID;              
                user.CreatedDate = CreatedDate;
                user.CreatedBy = CreatedBy;
                user.Weight = Weight;
                dbe.tbl_OutStockUserPackage.Add(user);
                int kq = dbe.SaveChanges();
                string k = user.ID.ToString();
                return k;
            }
