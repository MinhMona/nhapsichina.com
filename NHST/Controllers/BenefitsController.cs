...
namespace NHST.Controllers
{
    public class BenefitsController
    {
        ...
        public static tbl_Benefits Update1(int ID, string ServiceOfName, string ServiceOfContent, string ServiceOfIMG, bool IsHidden, int Position, string Created)
        {
            using (var db = new NHSTEntities())
            {
                var sv = db.tbl_Benefits.Where(x => x.ID == ID).FirstOrDefault();
                if (sv != null)
                {
                    sv.BenefitName = ServiceOfName;
                    sv.BenefitDescription = ServiceOfContent;
                    if (!string.IsNullOrEmpty(ServiceOfIMG))
                        sv.BenefitIMG = ServiceOfIMG;
                    sv.IsHidden = IsHidden;
                    sv.BenefitIndex = Position;
        ...
        public static List<tbl_Benefits> GetAllAD()
        {
            using (var db = new NHSTEntities())
            {
                var sv = db.tbl_Benefits.ToList();
                if (sv.Count > 0)
                    return sv;
                return null;
            }
