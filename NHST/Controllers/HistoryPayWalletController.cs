...
namespace NHST.Controllers
{
    public class HistoryPayWalletController
    {
        ...
        public static string Insert_AdminSendUserWalletID(int UID, string UserName, int MainOrderID, double Amount, string HContent, double MoneyLeft, int Type, int TradeType,
            DateTime CreatedDate, string CreatedBy, int AdminSendUserWalletID)
        {
            using (var dbe = new NHSTEntities())
            {
                dbe.Configuration.ValidateOnSaveEnabled = false;
                ...
                a.CreatedDate = CreatedDate;
                a.CreatedBy = CreatedBy;
                a.AdminSendUserWalletID = AdminSendUserWalletID;
                dbe.tbl_HistoryPayWallet.Add(a);
                int kq = dbe.SaveChanges();
                //string k = kq + "|" + user.ID;
                string k = a.ID.ToString();
        ...
        public static tbl_HistoryPayWallet GetByAdminSendUserWalletID(int id)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_HistoryPayWallet aus = new tbl_HistoryPayWallet();
                aus = dbe.tbl_HistoryPayWallet.Where(a => a.AdminSendUserWalletID == id).FirstOrDefault();
                return aus;
            }
        }
        ...
        public static int GetTotal_DK_statement(string fd, string td)
        {
            var sql = @"select Total=Count(*) ";
            sql += "from tbl_HistoryPayWallet ";
            if (!string.IsNullOrEmpty(fd))
            {
                var df = DateTime.ParseExact(fd, "dd/MM/yyyy HH:mm", null);
                sql += "where CreatedDate >= CONVERT(VARCHAR(24),'" + df + "',113) ";
            }
            if (!string.IsNullOrEmpty(td))
            {
                var dt = DateTime.ParseExact(td, "dd/MM/yyyy HH:mm", null);
                if (!string.IsNullOrEmpty(fd))
                    ...
                    sql += "where CreatedDate <= CONVERT(VARCHAR(24),'" + dt + "',113) ";
            }
            int a = 0;
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            while (reader.Read())
        ...
        public static List<tbl_HistoryPayWallet> GetBySQL_DK_statement_excel(string fd, string td)
        {
            var sql = @"select * ";
            sql += "from tbl_HistoryPayWallet ";
            if (!string.IsNullOrEmpty(fd))
        ...
        public static List<tbl_HistoryPayWallet> GetBySQL_DK_statement(string fd, string td, int pageSize, int pageIndex)
        {
            var sql = @"select * ";
            sql += "from tbl_HistoryPayWallet ";
            if (!string.IsNullOrEmpty(fd))
