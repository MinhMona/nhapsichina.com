...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                                ...
                                <div class="order-detail-wrap col s12 section account-sticky mt-2">
                                    <div class="row">
                                        <div class="col s12">
                                                            ...
                                                            <div class="order-action">
                                                                <div class="display-flex">
                                                                    <div class="action-left">
                                                                        ...
                                                                        <asp:Literal ID="ltrbtndeposit70" runat="server"></asp:Literal><asp:Button ID="btnDeposit70" runat="server" CssClass="btn pill-btn primary-btn" Style="display: none" UseSubmitBehavior="false" CausesValidation="false" Text="Đặt cọc" OnClick="btnDeposit70_Click" />
                                                                        <asp:Panel ID="pnthanhtoan" runat="server" Visible="false">
                                                                            <a href="javascript:;" style="display: none" class="btn btn-checkout" onclick="payallorder()">Thanh toán</a>
                                            ...
                                            <div class="card-panel">
                                                <div id="mvc-list2" class="section scrollspy mvc-list">
                                                    <div class="title-header bg-dark-gradient">
                                                    ...
                                                    <div class="responsive-tb">
                                                        <table class="table    highlight bordered  centered   ">
                                                            ...
                                                            <tbody class="list-product">
                                                                <asp:Literal runat="server" ID="ltrFeeSupport"></asp:Literal>
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="card-panel">
                                                <div class="row">
                                                    ...
                                                    <div class="col s12 m6" style="margin-top: 30px">
                                                        <div id="contact-chat" class="section chat-fixed" style="position: unset !important; width: 100%; border: 1px solid #ebebeb; padding: 0 !important;">
                                                            <div class="row">
                                                                <div class="col s12">
                                                                    <div class="title-header bg-dark-gradient">
                                                                        <h6 class="white-text ">
    ...
    <div id="modalOrder" class="modal modal-fixed-footer">
        <div class="modal-hd">
            <span class="right"><i class="material-icons modal-close right-align">clear</i></span>
            <h4 class="no-margin center-align">Đặt lại đơn hàng</h4>
        </div>
        <div class="modal-bd">
            <div class="row">
                <asp:Literal ID="ltrListSP" runat="server"></asp:Literal>
            </div>
        </div>
        <div class="modal-ft">
            <div class="ft-wrap center-align">
                <a href="javascript:;" onclick="OrderSP()" class="modal-action btn modal-close waves-effect waves-green mr-2 submit-button">Đặt hàng</a>
                <a href="javascript:;" class="modal-action btn orange darken-2 modal-close waves-effect waves-green ml-2">Hủy</a>
            </div>
    ...
    <asp:Button ID="btn_saveOrder" runat="server" OnClick="btn_saveOrder_Click" Style="display: none;" UseSubmitBehavior="false" />
    ...
    <asp:HiddenField ID="hdfOrderIDsp" runat="server" />
    <asp:HiddenField ID="hdfNumbersp" runat="server" />
    <asp:HiddenField ID="hdfSPinCount" runat="server" />
    <asp:HiddenField ID="hdfWarehouse" runat="server" />
    ...
    <script>
        ...
        function depositOrder(obj) {
            var r = confirm("Bạn muốn đặt cọc đơn hàng 90% này?");
            if (r == true) {
        ...
        function depositOrder70(obj) {
            var r = confirm("Bạn muốn đặt cọc đơn hàng 70% này?");
            if (r == true) {
                obj.removeAttr("onclick");
                $("#<%= btnDeposit70.ClientID%>").click();
            }
            else {
            }
        }
        function cancelOrder() {
            var r = confirm("Bạn muốn xóa đơn hàng này?");
            if (r == true) {
    ...
    <script>
        function OrderSP() {
            var number = $("#<%= hdfNumbersp.ClientID%>").val();
            var warehouse = false;
            var sp = false;
            var str = "";
            ...
            var warehouseID = $(".warehoseselect").val();
            var warehousefromID = $(".warehosefromselect").val();
            var shippingtype = $(".shippingtypesselect").val();

            if (warehouseID == 0 || warehousefromID == 0 || shippingtype == 0) {
                warehouse = true;
            }
            //var shippingtype = "1";
            var html = warehouseID + "-" + shippingtype + "-" + warehousefromID;

            if (sp || warehouse) {
                alert("vui lòng kiểm tra lại thông tin !!!");
            }
            else {
                $("#<%= hdfWarehouse.ClientID%>").val(html);
                $("#<%= hdfSPinCount.ClientID%>").val(str);
                $("#<%= btn_saveOrder.ClientID%>").click();
            }
        }
        <%--function Order(ID) {
            $("#<%= hdfOrderIDsp.ClientID%>").val(ID);
            $.ajax({
                type: "POST",
                url: "/chi-tiet-don-hang.aspx/loadinfoSP",
                data: '{ID: "' + ID + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (msg) {
    ...
    <script>
        function updateNote(ID) {
            var brand = $("#note_" + ID).val();
            $.ajax({
                type: "POST",
                url: "/chi-tiet-don-hang.aspx/UpdateNoteOrder",
