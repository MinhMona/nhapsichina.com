...
{
    public partial class TQ_OutStock : System.Web.UI.Page
    {
        ...
        public static string GetListPackage(string barcode)
        {
            ...
            if (HttpContext.Current.Session["userLoginSystem"] != null)
            {
                ...
                if (user != null)
                {
                    ...
                    {
                        var bigpackage = BigPackageController.GetByPackageCodeStatus(barcode, 1);
                        if (bigpackage != null)
                        {
                            ...
                            var smallpackages = SmallPackageController.GetBuyBigPackageIDNew(bID);
                        ...
                        else
                        {
                            ...
                            bi.BigpackageType = 2;
                            List<smallpackageitem> sis = new List<smallpackageitem>();
                            var smallpackages = SmallPackageController.GetListByOrderTransactionCodeOutChina(barcode);
                            if (smallpackages.Count > 0)
                            {
                                foreach (var item in smallpackages)
                                {          
           ...
           string nvkiemdem, string khachghichu, string loaisanpham)
        {
            ...
            {               
                ...
                {                   
                    SmallPackageController.UpdateStaffNoteCustdescproducttype(package.ID, nvkiemdem, khachghichu, loaisanpham);
                    SmallPackageController.UpdateNote(package.ID, note);
                    SmallPackageController.UpdateStatus(package.ID, status);
                    ...
                    if (mainorder != null)
                    {
                        int orderID = Convert.ToInt32(mainorder.ID);
                        int SttOrder = Convert.ToInt32(mainorder.Status);
                        ...
                        if (SttOrder < 8)
                        {
                            if (mainorder.DateToVN == null)
                            {
                                MainOrderController.UpdateDateToVN(orderID, currentDate);
                            }
                            MainOrderController.UpdateStatus(orderID, Convert.ToInt32(mainorder.UID), 7);
                            HistoryOrderChangeController.Insert(mainorder.ID, accChangeData.ID, accChangeData.Username, accChangeData.Username +
                            " đã đổi trạng thái đơn hàng của bạn sang Đang trên đường về kho VN", 8, currentDate);
                            ...
                            if (setNoti != null)
                            {
                                ...
                                if (acc != null)
                                {
                                    if (setNoti.IsSentNotiUser == true)
                                    {
                                        ...
                                        acc.Username, orderID, "Hàng của bạn Đang trên đường về kho VN.", 1, currentDate, username_current, true);
                                    }
                                }
                            }
                        }    
                                        ...
                                        if (order != null)
                                        {
                                           ...
                                           0, temp, "", 0, 0, 0, 3, Description, DateTime.Now, username, ordershopcode.ToInt(0), 0);
                                                ...
                                                if (smallpackages.Count > 0)
                                                {
                                                    ...
                                                    if (isChuaVekhoTQ == false)
                                                    {
                                                        MainOrderController.UpdateStatus(mainOrderID, Convert.ToInt32(order.UID), 7);
                                                    }
                                                }
                                                JavaScriptSerializer serializer = new JavaScriptSerializer();
