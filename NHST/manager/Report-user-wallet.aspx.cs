...
namespace NHST.manager
{
    public partial class Report_user_wallet : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            if (st == "3")
            {
                var la = AccountController.GetAllOrderByWalletDesc(10, page);
                var total = AccountController.GetTotalOrderByWalletDesc();
                double WalletAll = AccountController.GetTotalWalletDesc();
                lbTotalWallet.Text = string.Format("{0:N0}", WalletAll) + " VND";
                pagingall(la, total);
            }
            if (st == "4")
            {
                var la = AccountController.GetAllOrderByWallet(10, page);
                var total = AccountController.GetTotalOrderByWalletDesc();
                double WalletAll = AccountController.GetTotalWalletDesc();
                lbTotalWallet.Text = string.Format("{0:N0}", WalletAll) + " VND";
                pagingall(la, total);
