...
namespace NHST.manager
{
    public partial class ContactCustomer : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            var total = ContactCustomerController.GetTotal(search, phone, status);
            var la = ContactCustomerController.GetAllBySQL(search, phone, page, 20, status);
            pagingall(la, total);
        }
        ...
        public static string LoadInfor(string ID)
        {
            ...
            var com = ContactCustomerController.GetByID(ID.ToInt(0));
            if (com != null)
            {
                ContactListShow l = new ContactListShow();
                if (com != null)
                {
                    l.FullName = com.FullName;
                    l.Email = com.Email;
                    l.Note = com.Note;
        ...
        public void pagingall(List<tbl_ContactCustomer> acs, int total)
        {
            int PageSize = 20;
            if (total > 0)
            {
        ...
        public static string UpdateStatus(int ID)
        {
            ...
            var check = ContactController.GetByID(ID);
            DateTime currentDate = DateTime.Now;
            if (check != null)
            {
                ContactCustomerController.Update(check.ID, 2, DateTime.Now, username);
                return "ok";
            }
            else
                return "none";
