...
namespace NHST.manager
{
    public partial class User_Level : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var uLevel = UserLevelController.GetByID(ID);
            ...
            if(uLevel.LessDeposit != Convert.ToDouble(txtLessDeposit.Text))
            {
                var u = AccountController.GetByLevelID(ID);
                if(u != null)
                {
                    foreach(var item in u)
                    {
                        if(item.Deposit == 0)
                        {
                            var Mainoder = MainOrderController.GetByUIDAndStatus(item.ID, 0);
                            foreach(var temp in Mainoder)
                            {
                                var MoneyDeposit = Convert.ToDouble(temp.PriceVND) * Convert.ToDouble(txtLessDeposit.Text) / 100;
