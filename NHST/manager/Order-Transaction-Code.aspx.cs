    ...
    public partial class Order_Transaction_Code : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["w"]))
            {
                ...
                ddlWare.SelectedValue = Request.QueryString["w"].ToString();
            }
            int hasVMD = 0;
            ...
            if (ac.RoleID == 9)
            {
                var total = SmallPackageController.GetTotalCSKH(stype, search, st, fd, td, w, ac.ID, Convert.ToBoolean(hasVMD));
                var la = SmallPackageController.GetAllSQLCSKH(stype, search, st, fd, td, page, 20, w, ac.ID, Convert.ToBoolean(hasVMD));
                pagingall(la, total);
            }    
            else
            {
                var total = SmallPackageController.GetTotalBy_DK(stype, search, st, fd, td, w, Convert.ToBoolean(hasVMD));
                var la = SmallPackageController.GetAllSQLHelperWith_DK(stype, search, st, fd, td, page, 20, w, Convert.ToBoolean(hasVMD));
                pagingall(la, total);
            } 
        }
        ...
        public void pagingall(List<SmallPackageController.smallpackage> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    if (MainOrderController.GetByID(item.MainOrderID) != null)
                    {
                        hcm.Append("<td>" + WarehouseController.GetByID(MainOrderController.GetByID(item.MainOrderID).ReceivePlace.ToInt(0)).WareHouseName + "</td>");
                    }
                    else if (TransportationOrderNewController.GetByID(item.TransportationOrderID) != null)
                    {
                        hcm.Append("<td>" + WarehouseController.GetByID(TransportationOrderNewController.GetByID(item.TransportationOrderID).WareHouseID.Value).WareHouseName + "</td>");
                    }    
                    else
                    {
                        hcm.Append("<td></td>");
                    ...
                    hcm.Append("<td>" + item.STT + "</td>");
                    hcm.Append("<td>" + item.DateExcel + "</td>");
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string ware = ddlWare.SelectedValue;
            ...
            if (string.IsNullOrEmpty(searchname) == true && fd == "" && td == "" && st == "" && stype == "0" && ware == "0" && hasVMD == "0")
            {
                Response.Redirect("Order-Transaction-Code");
            }
            else
            {
                Response.Redirect("Order-Transaction-Code?stype=" + stype + "&s=" + searchname + "&fd=" + fd + "&td=" + td + "&st=" + st + "&w=" + ware + "&hasMVD=" + hasVMD);
            }
        }
        #endregion
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            var la = SmallPackageController.GetAllSQLHelperWith_DK(stype.ToString().ToInt(0), search, st, fd, td, 0, 0, w, Convert.ToBoolean(hasVMD));
            ...
            StrExport.Append("      <th><strong>Kho đích</strong></th>");
            ...
            foreach (var item in la)
            {
                ...
                if (MainOrderController.GetByID(item.MainOrderID) != null)
                {
                    StrExport.Append("<td>" + WarehouseController.GetByID(MainOrderController.GetByID(item.MainOrderID).ReceivePlace.ToInt(0)).WareHouseName + "</td>");
                }
                else if (TransportationOrderNewController.GetByID(item.TransportationOrderID) != null)
                {
                    StrExport.Append("<td>" + WarehouseController.GetByID(TransportationOrderNewController.GetByID(item.TransportationOrderID).WareHouseID.Value).WareHouseName + "</td>");
                }
                else
                {
                    StrExport.Append("<td></td>");
                }
                StrExport.Append("      <td>" + item.BigPackage + "</td>");
                ...
                StrExport.Append("      <td>" + Math.Round(Convert.ToDouble(item.Volume), 2) + "</td>");
