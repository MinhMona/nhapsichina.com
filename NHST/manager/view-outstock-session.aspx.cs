...
namespace NHST.manager
{
    public partial class view_outstock_session : System.Web.UI.Page
    {
                    ...
                    if (os != null)
                    {
                        ...
                        if (ops.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                //    ltrBtnUpdate.Text = "<a href=\"javascript:;\" class=\"btn mr-2\" onclick=\"UpdateOrder()\">Cập nhật</a>";
                                //}
                            }
                            //if (totalPriceMustPay > 0)
                            //{
        ...
        protected void btncreateuser_Click(object sender, EventArgs e)
        {
            ...
            if (id > 0)
            {
                ...
                if (ots != null)
                {
                    ...
                    AccountantOutStockPaymentController.Insert(ots.ID, totalPay, Convert.ToInt32(ots.UID), ots.Username, "Xuất kho chưa thanh toán", currentDate, username_current);
                    OutStockSessionController.updateInfo(id, txtFullname.Text, txtPhone.Text);
                    OutStockSessionController.updateStatus(id, 2, currentDate, username_current);
                    OutStockSessionController.updateCheckPrice(id, false);
                    var c = ConfigurationController.GetByTop1();
                    string content = ViewState["content"].ToString();
                    var html = "";
                    ...
                    html += "           <label class=\"row-info\">Công nợ</label>";
                    html += "       </div>";
                    html += "       <div class=\"bill-row\">";
        ...
        {
            var check = Convert.ToBoolean(chkPriceto.Value.ToInt(0));
            if (check)
            {
                int id = ViewState["id"].ToString().ToInt(0);
                if (id > 0)
                {
                    ...
                    if (os != null)
                    {
                        OutStockSessionController.updateCheckPrice(id, check);
                        var osp = OutStockSessionPackageController.GetAllByOutStockSessionID(id);
                        if (osp != null)
                        {
                            foreach (var item in osp)
                            {
                                var small = SmallPackageController.GetByID(item.SmallPackageID.Value);
                                if (small != null && (small.Status == 4 || small.Status == 6))
                                {
                    ...
                    List<int> mainIDs = mainID.Distinct().ToList();
                    foreach (var item in mainIDs)
                    {
                        var countSmall = SmallPackageController.GetByMainOrderID(item).Count();
                        var countSmallStatus = SmallPackageController.GetByMainOrderIDAndStatus(item, 4).Count() + SmallPackageController.GetByMainOrderIDAndStatus(item, 6).Count();
                        if (countSmall == countSmallStatus)
                        {
                            MainOrderController.UpdateStatusByID(item, 9);
                        }
                    }
                }
            }
                                        ...
                                        if (o != null)
                                        {
                                            ...
                                            if (obj_user != null)
                                            {
                                                ...
                                                if (wallet >= leftpay)
                                                {
                                                    ...
                                                    PayOrderHistoryController.Insert(id, UID, 9, leftpay, 2, currentDate, username_current);
                                                    OutStockSessionController.updateStatus(id, 2, currentDate, username_current);
                                                    var smalls = SmallPackageController.GetByMainOrderID(o.ID);
                                                    var a = OutStockSessionPackageController.GetAllByOutStockSessionIDAndMainOrderID(ots.ID, o.ID);
                                                    if (a.Count > 0)
                                                    {
                                                    ...
                                                    if (o.IsDebt == true)
                                                    {
                                                        MainOrderController.UpdateIsDebt(o.ID, false);
                                                    }
                                                    ...
                                                    //}
                                                    HistoryOrderChangeController.Insert(o.ID, userAdmin.ID, username_current, username_current +
                                                                " đã đổi trạng thái của đơn hàng ID là: " + o.ID + ", từ: Đã đang giao hàng, sang: Khách đã thanh toán.", 1, currentDate);
                                                    MainOrderController.UpdateStatusByID(o.ID, 9);
                                                    PayOrderHistoryController.Insert(o.ID, Convert.ToInt32(o.UID), 9, leftpay, 2, currentDate, username_current);
                                                    if (o.PayDate == null)
                                                    {
