                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    if (this.currFileExtension == ".xlsx" || this.currFileExtension == ".xls")
                    {
                        ...
                        if (_FileTempPlan != null)
                        {
                            ...
                            foreach (DataRow drRow in _FileTempPlan.Rows)
                            {
                                try
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(checkMVD))
                                    {
                                        ...
                                        {
                                            SmallPackageController.InsertTroiNoiKhoVN(checkMVD, Weight, obj_user.Username, currentDate, currentDate, obj_user.Username);
                                        }
                                        else
                                        {
                                            checkMVD += drRow["MaVanDon"].ToString().Trim() + " - ";
                            ...
                            if (checktb)
                            {
                                PJUtils.ShowMessageBoxSwAlert("Import thành công kho Việt Nam " + t + " mã vận đơn.", "s", true, Page);
                            }
                            else
                            {
