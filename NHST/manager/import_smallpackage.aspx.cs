                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    if (this.currFileExtension == ".xlsx" || this.currFileExtension == ".xls")
                    {
                        ...
                        if (_FileTempPlan != null)
                        {
                                    ...
                                    if (!string.IsNullOrEmpty(checkMVD))
                                    {
                                        ...
                                        if (check != null)
                                        {
                                            if (check.Status < 3)
                                            {
                                                ...
                                                SmallPackageController.UpdateImportKhoTQ(smallID, Weight, 2, obj_user.Username, currentDate, obj_user.Username, currentDate, BaoLon, STT);
                                                if (MainOrderID > 0)
                                                {
                            ...
                            if (checktb)
                            {
                                PJUtils.ShowMessageBoxSwAlert("Import thành công kho Trung Quốc " + t + " mã vận đơn.", "s", true, Page);
                            }
                            else
                            {
