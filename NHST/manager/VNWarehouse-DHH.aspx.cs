                    ...
                    if (mainorder != null)
                    {
                        ...
                        foreach (var item in smallpackage)
                        {
                            if (item.Status == 3 || item.Status == 4 || item.Status == 6)
                            {
                                ...
                                if (weight >= compareSize)
                                {
                                    TongCanNang += Math.Round(weight, 5);
                                    additonWeight += Math.Round(weight, 5);
                                    if (item.Status != 3)
                                    {
                                        weightPaid += Math.Round(weight, 5);
                                    }
                                }
                                else
                                {
                        ...
                        if (accChangeData != null)
                        {
                            ...
                            if (checkIsChinaCome == true)
                            {
                                ...
                                int Status = Convert.ToInt32(mainorder.Status);
                                int UID = Convert.ToInt32(mainorder.UID);
                                if (Status < 9)
                                {
                                    ...
                                    MainOrderController.UpdateStatus(MainorderID, UID, 8);
                                    HistoryOrderChangeController.Insert(mainorder.ID, accChangeData.ID, accChangeData.Username,
                                    accChangeData.Username + " đã đổi trạng thái đơn hàng ID là: " + mainorder.ID + ", là: Đã về kho đích", 8, currentDate);
                                    ...
                                    if (setNoti != null)
                                    {
                                        ...
                                        if (acc != null)
                                        {
                                            ...
                                            if (setNoti.IsSendEmailUser == true)
                                            {
                                                try
                                                {
                                                    PJUtils.SendMailGmail("nslogi.vietnam@gmail.com", "szbenufftlagjslh",
                                                    acc.Email, "Thông báo tại hệ thống nhập hàng Trung Quốc NHAPSICHINA.", "Đơn hàng mua hộ " + MainorderID + " hàng đã về kho Hà Nội.", "");
                                                }
                                                catch { }
                                            }
                                        }
