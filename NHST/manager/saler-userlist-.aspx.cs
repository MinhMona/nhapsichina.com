...
{
    public partial class saler_userlist_ : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (ac != null)
            {
                var la = AccountController.GetListStaffOfSaleBySQL(search, ac.ID, page, 20);
                var total = AccountController.GetTotalStaffOfSale(search, ac.ID);
                pagingall(la, total);
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("saler-userlist-?s=" + searchname);
            }
            else
            {
                Response.Redirect("saler-userlist-");
            }
        }
        protected void btnAddSaler_Click(object sender, EventArgs e)
        {
            string username_current = Session["userLoginSystem"].ToString();
            tbl_Account ac = AccountController.GetByUsername(username_current);
            int id = hdfIDuser.Value.ToInt();
            var user = AccountController.GetByID(id);
            if (user.SaleID != 0)
            {
                AccountController.UpdateSaleID(id, 0, DateTime.Now, ac.Username);
                PJUtils.ShowMsg("Trả khách thành công.", true, Page);
            }
            else
            {
                PJUtils.ShowMessageBoxSwAlert("Trả khách không thành công.", "e", true, Page);
            }
        }
        #endregion
        ...
        public void pagingall(List<View_UserList> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    if (item.SaleID == 0)
                        hcm.Append("<td> Bạn chưa có quyền xem </td>");
                    else
                    ...
                    if (item.SaleID != 0)
                        hcm.Append("<p style=\"cursor: pointer; background-color: #366136; color: white; border-radius: 5%;\" class=\"button-phuong\" onclick=\"AddSaler('" + item.ID + "')\">Trả khách</p>");
                    hcm.Append("</div>");
                    hcm.Append("</td>");
