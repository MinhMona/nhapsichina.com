    ...
    public partial class sale_userlist : System.Web.UI.Page
    {
        ...
        private void LoadData()
        {
            ...
            if (ac != null)
            {
                ...
                {
                    var la = AccountController.GetListStaffOfCSKH(search, ac.ID, page, 20);
                    var total = AccountController.GetTotalStaffOfCSKH(search, ac.ID);
                    pagingall(la, total);
                }
            }
        ...
        protected void btnAddSaler_Click(object sender, EventArgs e)
        {
            ...
            if (user.SaleID == 0)
            {
                AccountController.UpdateSaleID(id, ac.ID, DateTime.Now, ac.Username);
                PJUtils.ShowMsg("Cập nhật thông tin thành công.", true, Page);
            }
            else
            {
                PJUtils.ShowMessageBoxSwAlert("Đã có nhân viên khác nhận khách.", "e", true, Page);
            }
        }
        #endregion
        ...
        public void pagingall(List<View_UserList> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    {
                        if (item.SaleID == 0)
                            hcm.Append("<p style=\"cursor: pointer; background-color: #366136; color: white; border-radius: 5%;\" onclick=\"AddSaler('" + item.ID + "')\">Add khách</p>");
                    }
                    hcm.Append("</td>");
                    hcm.Append("</tr>");
                }
