...
namespace NHST
{
    public partial class user_outstock_payment : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            var la = OutStockSessionController.GetBySQL_Masu(searchname, page, 20, fd, td, status, ac.ID);
            int Total = OutStockSessionController.GetTotalBySQL_Masu(searchname, fd, td, status, ac.ID);
            ...
            if (!string.IsNullOrEmpty(searchname))
            {
                if (!string.IsNullOrEmpty(fd) && !string.IsNullOrEmpty(td))
                {
                    ...
                    else
                    {
                        ...
                        Response.Redirect("user-outstock-payment?s=" + searchname + "&fd=" + fd + "&td=" + td + "&st=" + st + "");
                    }
                }
                else
                {
                    Response.Redirect("user-outstock-payment?st=" + st + "&s=" + searchname);
                }
            ...
            else
            {
                if (!string.IsNullOrEmpty(fd) && !string.IsNullOrEmpty(td))
                {
                    ...
                    else
                    {
                        ...
                        Response.Redirect("user-outstock-payment?fd=" + fd + "&td=" + td + "&st=" + st + "");
                    }
                }
                else
                {
                    Response.Redirect("user-outstock-payment?st=" + st + "");
                }
            }
        }
        ...
        public void pagingall(List<OutStockSessionController.OutStockNew> acs, int total, int PageSize)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    hcm.Append("<a href=\"viewuser-outstock-session.aspx?id=" + item.ID + "\" class=\"tooltipped\" data-position=\"top\" data-tooltip=\"In phiếu\"><i class=\"material-icons\">edit</i><span>In phiếu</span></a>");
                    hcm.Append("</div>");
                    hcm.Append("</td>");
                    hcm.Append("</tr>");
                }
